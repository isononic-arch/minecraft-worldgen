...
**Current state:** Session 49 (2026-04-13) — **Phase 2.0 + 2.5 code complete + S49 bugfixes landed.** 6 layers unchanged. S49 fixes: (1) BIOME_TO_MC consolidated to single canonical source in `chunk_writer.py` — 4 duplicate dicts removed, ALPINE_MEADOW→meadow, EASTERN_TEMPERATE_COAST→beach; (2) ground cover slope-gated (0% cliff, 10% talus, 50% moderate) + post-pipeline suppression on non-plantable surfaces; (3) lithology pushed 1 block deeper — stone_mask now `sy-4`, 3 blocks sub_blk buffer, geology invisible at convex edges; (4) **cliff_deg staircase aliasing fix** — new `compute_cliff_deg(sigma=1.5)` Gaussian-smooths surface_y before gradient; eliminates contour-line banding on gentle slopes; all 5 call sites updated. Stone faces only via ≥35° cliff layer. 77/77 tests green. **NEXT (S50):** Regenerate (24,80) to validate S49 fixes in-game. Then Phase 2.75 (water-adjacent + cross-biome). **Carry-forward:** aspect convention drift, 51_53 flag-ON shadow hookup, palette tuning, ice in subsurface (user expects topsoil update to fix), world_studio.py duplicate BIOME_TO_MC + missing surface pipeline params.
