...
**Current state:** Session 57 (2026-04-16) — **Surface simplification + zone 40 biome fix (Phase 2.75e).** Eco overlay loop disabled (eco_ridge/eco_moist/etc. no longer re-applied on top of noise_layers — eliminates banding for all biomes). GrassTerrace archived (`core/layers/_archived/`). Zone 40 MC biome inheritance fixed — EDT now correctly identifies zone 40 pixels via override_tile instead of dead `== "ALPINE_MEADOW"` check; adjacent-lowland biome (e.g. minecraft:desert near deserts) propagates via `alpine_biome_source`. Grass cutoff at elevation added (Y 325→350 fade, biome-agnostic). **Carry-forward:** rock lithology palette softening (deferred to final cleanup), cross-tile ecotone at seams, SEMI_ARID_SHRUBLAND sand patches inland, desert_pavement ground cover, Phase B height.tif regen from Erosion2_Out, aspect convention drift, world_studio.py duplicate BIOME_TO_MC, snow tuning (peak detector coverage).
