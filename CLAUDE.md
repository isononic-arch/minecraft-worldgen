...
**Current state:** Session 93→93e2 (2026-06-12) — **SHIPPED through `8e66502` + config-enabled taper (S93e2: first install FRAGMENTED streams — gates measured rm/rwy assignments not RENDERED water; box-fixed with min_tube_carve floor + paint-skeleton-UNION BFS, gated on TRUE-WET CONNECTIVITY (1 component/tile, all 6 pass), variant B knobs (hw_min 2.5/k 0.45/floor 1.4) live in `config.river_carve.headwater_taper`, code default OFF, PENDING WALK; S93d razor-seam user-CONFIRMED; rivers user-CONFIRMED) — READ [memory/S93_handoff.md](memory/S93_handoff.md) FIRST (BED-CACHE LANDMINE + S93c addendum: canyon guard `bed_max_extra_depth` for the pre-768 absolute-Y bed (dug 225-deep at Y500, byte-no-op at lowland); cross-tile ecotone dither for GC+tree species (padded fields + world-coord hashed GC picks); windowed 8k sampling (512MB/call prefilter → workers were getting OOM-killed silently — local box is 7.5GB, render 1-worker locally)).** S93 rework of the S92 reverts, one change per walk: (1) tree-border lane fix via edge-guard SUBSTITUTION (re-roll among fitting entries) — user-walked, fine; (2) carver MONOTONE water-y quantization (kills checkered/floating water at elevation steps; (27,34) incoherent 34→0) — user-walked, good; (3) lake-bowl depth warp (bed terraces arc with the shoreline instead of straight x/z ledges) — gate-passed, PENDING USER WALK at (33,33)/(62,61). **CRITICAL: `masks/_bed_cache_v17.pkl` is LOAD-BEARING + IRREPRODUCIBLE (NEVER delete) — current bed builder rebuilds a SHALLOWER bed (streams depth 4-5→1, estuaries vanish). Shipped: race-safe v17→v19 verbatim migration + `_BedCacheRefusal` (any rebuild while v17 exists is FATAL, incl. stale-key path). Before any box/50k: log must show `bed cache MIGRATED`/`HIT`, and healthy v19 ≈ 1.43GB (a ~1.1GB v19 = poisoned rebuild → delete + re-warm).** Headwater thin→wide PARKED behind `_HW_INSET_ENABLED=False` (needs carver width-map session). Local renders: ≤2 workers (4 OOM'd; pipeline exits 0 on tile errors — grep `tile_error`). Bed-builder reconciliation vs v17 = own session. — Prior S92 revert postmortem: [memory/S92_handoff.md](memory/S92_handoff.md). Code = S91 tip `6b26cd7` exactly (bed cache back on v17). S92's river fixes (tidal clamp/coherence/headwater taper) and tree-seam fixes (substitution + border band) all walked BADLY (straggler water at steps, crossguard floating water, lake-border straggler, forest border bands/stripes worse than the S91 lane) despite passing every numeric gate — rework must be smaller-scale, one change per walk, with TOP-DOWN VISUAL diffs as the gate. The S92 walk DID confirm S91 wins: high-relief 73|74 seam FIXED (the seam seen near z≈34304 is vs STALE (73,67)/(74,67) neighbors), snowy massif + litho-at-elevation great. River geometry audit findings remain valid (depth ≤5 everywhere = no trenches; headwaters have a ~9-block paint-floor width — taper rework needs the carver width-map session first). — Prior S91 state (5 regressions, all fixed+walked): [memory/S91_handoff.md](memory/S91_handoff.md), summarized next paragraph.
