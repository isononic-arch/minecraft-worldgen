...
**Current state:** Session 85 (2026-05-22/23) — **Reconciliation merge**: local master was 32 commits behind origin/master (stuck at S69 while origin advanced through S70-S84).  Cherry-picked S70-S84 work plus S85 surgical edits onto a feature branch `s85-cherry-picks` (commits `a738fb4` checkpoint + `8dca21b` cherry-pick batch).  Brought in: bbox-cull perf (3.3× speedup), datapack auto-install (assets→output/datapacks), AT high-elev stone-fade exemption + fade band 500-580, AT GC palette refresh, diag_mca_biomes/surface tools, FROZEN_FLATS Tundra Valley redesign (grass_block surface + scattered snow_carpet + sparse pines + badlands MC tag), lithology mask as source-of-truth for cliff + high-elev fade, ecotone plateau-clamp (width 40→100, swap_cap 0.75→0.85 — "incredibly wide transitional biomes"), schematic placement density tuning per S70-S81 walk-feedback, leaf-column blacklist (4 schematics rejected), SARID juniper-only filter, BIRCH clumping, floodplain biome skip for RIPARIAN/FRESHWATER_FEN/LUSH/SAND_DUNE_DESERT, AT+FF added to `_NO_SWAP_BIOMES`, S71-S83 river overhaul (WP-style carve, tanh depth, coast taper, lake-bowl geomorph), disk-pickle bed cache.
