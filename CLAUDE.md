...
**Current state:** Session 71 (2026-04-27/28) — multi-day arc, **major package landed in single commit**.  Highlights: (a) WP-style river overhaul ported from `river_script1.7.js` — guardrails carve + radius-averaged terrain + per-pixel `water_y_field` carry-through + delta connectivity (steepest-descent extension to ocean) + meander (universal incl. connectivity channels) + iterative gravity + slope-aware width + edge water-skip + fixify spike-relaxation; (b) lithology source-of-truth fix in surface_decorator (cliff/fade now uses painted lithology mask, biome `zone_to_group` is fallback only); (c) FF↔AT concept swap — FROZEN_FLATS = "Tundra Valley" lowland permafrost meadow (`minecraft:badlands` MC tag, scattered snow_carpet GC, NO snow on tree leaves, smallest pines via mirror, white-noise surface scatter), ARCTIC_TUNDRA = harsh mountain-snowy with snowgap applied; (d) NOISE_PATTERNS §4 plateau clamp `[0.15, swap_cap]` for ecotone dither (replaces ramp-to-zero formula); (e) BIRCH_FOREST clumping fix (no-repeat history 30→60, suppress weight 0.01→0.0001, species-level dedupe, jitter ±2→±3); (f) leaf-column blacklist (4 schems: dstep_acacia_a/c, dosav_soak_a/b); (g) SARID spruce-only filter; (h) BIOME_TO_MC overhauls — COASTAL_HEATH→savanna_plateau, BOREAL_TAIGA→stony_shore, CONTINENTAL_STEPPE→cherry_grove, SCRUBBY_HEATHLAND→badlands, FROZEN_FLATS→badlands, RIPARIAN_WOODLAND→mangrove_swamp + BIOME_ALTITUDE_REMAPS removed BT (was unconditionally remapping to BA→plains).  **CRITICAL CARRY-FORWARD:** river slope-water issue NOT YET CONFIRMED FIXED — full WP package landed at session-end, COASTAL_HEATH (37,8) post-S71-final-river not walked yet.  Full session log in [memory/S71_handoff.md](memory/S71_handoff.md) and §18 S71 entry.
