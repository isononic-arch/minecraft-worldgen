...
**Current state:** Session 46 (2026-04-11) — **Phase 1.5 "Lithology wiring scaffolding" IN PROGRESS.** S45 landed Phase 0.75 shadow-mode hookup. S46 began Phase 1 but discovered a **spec-vs-code architectural drift**: §11 Phase 1 originally described extending `column_generator.fill_column(...)` with lithology params, but `fill_column()` does not exist, and `column_generator` emits only a sparse `ColumnResult.blocks` dict (bedrock + sy-2/sy-1/sy/water) that the .mca path never reads for mid-column fill. The real mid-column injection point is `core/chunk_writer.build_column_array()` (line 202), which builds the full `(Y_RANGE, H, W)` volume with bedrock, biome-aware cliff stone banding, subsurface, surface, ground cover, and water fill. **Resolution:** surfaced drift to user, user picked "fatten the real path"; §11 Phase 1 preserved verbatim but marked SUPERSEDED, new **Phase 1.5** subsection inserted targeting `build_column_array()` instead. Phase 1.5 scope: add 4 optional kwargs (`lithology_tile`, `sediment_thickness_tile`, `soil_horizon_depth_tile`, `use_new_geology`) to `build_column_array()` + pass-through in `process_tile_columns_v2` / `generate_columns`; early-out guard preserves flag-OFF byte identity; flag-ON raises `NotImplementedError`; 5 new unit tests. **Tile choice correction:** Claude initially picked 36_20 as a "temperate mountain" reference — user corrected (36_20 is desert). Repicked **59_53** (windthrow / temperate mountain) as the new 3×3 baseline tile. 59_53 baseline snapshot running in background during doc work, blocking-prereq to any code edit. **NEXT:** finish 59_53 baseline → code edits (chunk_writer + column_generator pass-through) → 5 unit tests → 3×3 flag-OFF validation on 59_53, 48_48, 51_53. **Prereq:** 59_53 baseline MUST complete and be snapshotted to `tests/baselines/3x3/59_53/` before first edit to `core/chunk_writer.py` or `core/column_generator.py`.
