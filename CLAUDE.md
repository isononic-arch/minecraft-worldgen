...
**Current state:** Session 54 (2026-04-14) — **Two slope-driven surface bugs fixed.** (1) Stone contour-line banding ROOT CAUSE: `run_pipeline.py` computed `cliff_deg` from raw `np.gradient(surface_y)` without Gaussian smoothing — every 1-block terrain step read as 45°, firing `temperate_cliff_face` (≥35°) and `temperate_talus_apron` (18-35°) at every contour line. Fix: replaced with `compute_cliff_deg(surface_y)` (sigma=1.5 Gaussian pre-smooth). In-game validated — **flat-terrain stone banding eliminated.** See `memory/CONTOUR_BANDING_FIX.md`. (2) River bank cobble/gravel: cliff_face and talus_apron fired on carved river channel banks (2-4 block drops exceed thresholds even after smoothing). Fix: added `riparian_proximity >= 0.3` exclusion to both layers' scope masks. In-game validated — **river banks restored to normal riparian dither.** **NEXT:** Ecotone dither width (24px grass bleed at biome boundaries), beach blob/staircasing on (35,77), desert pavement ground cover. **Carry-forward:** aspect convention drift, 51_53 flag-ON shadow hookup, palette tuning, ice in subsurface, world_studio.py duplicate BIOME_TO_MC + missing surface pipeline params, desert_pavement ground cover palettes (dead_bush/short_dry_grass/tall_dry_grass), full column_generator surface code cleanup (steps 5-10 write to dead surface_blk).
