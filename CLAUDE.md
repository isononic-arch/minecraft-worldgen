...
**Current state:** Session 89 (2026-06-01) — **S89 rock+talus+cliff_cap+physics-snow + foliated-gully + DEPTH-SNOW surface system LIVE behind flags.**  v22 (foliated-gully mid-tier network + normalized washes) rendered + verified in-world.  **NEW depth-snow redesign staged (not yet rendered):** `snow_potential.tif` continuous potential (built by `build_snow_physics.py` alongside the binary gap) drives snow DEPTH — `_apply_depth_snow` in surface_decorator paints `P>=t_block`→snow_block CAP / `t_layer<=P<t_block`→snow[layers=N] DRIFT (deepening) / below→bare rock; carpet_biomes keep clean layers=1 baseline; steep faces bare; **kills the 95% blue-noise dapple** (shape from terrain, not noise).  Config `snow_physics.depth` (enabled, t_layer 0.35 / t_block 0.62 / max_layers 7 / slope_max 38).  Falls back to legacy carpet if mask missing.  Branch `s85-cherry-picks`.  New surface system is feature-flagged (`lithology.rock_layers.enabled` + `snow_physics.enabled`, committed OFF; `cloud_bake/render_s89_rocksnow.sh` flips them ON box-locally).  rock_layers slope tiers **dark 38° / mid 45° / light 50°** (`smax_deg=90` is the saturation ceiling, mostly cosmetic; `fade_to_deg` was a dead key, removed).  Per-group palettes (rock dark/mid/light, talus fine/coarse, cap, wash) repainted to user spec v10.  talus toggled OFF (`rock_layers.overlays.talus=false`) — re-enable for next render.  Fade-band root cause + fix: see antipattern #10 below.  **Next: user auditing all 6 litho tiles in-world for palette/slope tuning (config-only knobs).**  Prior S87 BT-banding notes: [memory/S87_to_S88_handoff.md](memory/S87_to_S88_handoff.md).
