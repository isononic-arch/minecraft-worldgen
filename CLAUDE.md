...
**Current state:** Session 92 (2026-06-10) — **ALL S92 CODE REVERTED (`3cd7376`) — READ the POSTMORTEM at the top of [memory/S92_handoff.md](memory/S92_handoff.md) FIRST.** Code = S91 tip `6b26cd7` exactly (bed cache back on v17). S92's river fixes (tidal clamp/coherence/headwater taper) and tree-seam fixes (substitution + border band) all walked BADLY (straggler water at steps, crossguard floating water, lake-border straggler, forest border bands/stripes worse than the S91 lane) despite passing every numeric gate — rework must be smaller-scale, one change per walk, with TOP-DOWN VISUAL diffs as the gate. The S92 walk DID confirm S91 wins: high-relief 73|74 seam FIXED (the seam seen near z≈34304 is vs STALE (73,67)/(74,67) neighbors), snowy massif + litho-at-elevation great. River geometry audit findings remain valid (depth ≤5 everywhere = no trenches; headwaters have a ~9-block paint-floor width — taper rework needs the carver width-map session first). — Prior S91 state (5 regressions, all fixed+walked): [memory/S91_handoff.md](memory/S91_handoff.md), summarized next paragraph.
