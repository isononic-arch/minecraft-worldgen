...
**Current state:** Session 101 (2026-07-02) — **PIPELINE 5-6× FASTER (every change byte-identity-gated) + ISLAND PLACEMENT FINALIZED. READ [memory/S101_perf_audit.md](memory/S101_perf_audit.md) FIRST** (+ the S100/S101 close-out sections in the auto-memory S99 handoff). (1) **Perf, all byte-gated:** vectorized OpenSimplex `core/fast_simplex.py` (bitwise 49/49 vs reference; class-patch installed in run_pipeline + validators; `VANDIR_FAST_NOISE=0` disables), chunk_writer NBT-emit + stamp_schematic index-domain reworks, hydro_region_overlay exact numpy scanline replacing matplotlib points_in_path (pixel-identical; `VANDIR_SLOW_RASTER=1` fallback). `r.21.30.mca` md5 IDENTICAL pre/post with AND without trees. Dense tile: 200-320s class → **37.8s bare / 50.6s with 4,898 trees**. Equality harnesses `tools/diag_{fast_simplex,nbt_emit,river_raster}_equiv.py` — RE-RUN after touching those paths (or an opensimplex/matplotlib version bump). (2) **Islands:** all 15 masks re-baked S101 (unified S98 ocean cfg, tree_seam band_pass inherited, Efate warm-temperate reband, Fogo (21418,-7993), Madre z=-8888 with the apron cull FIXED in `islands/_box_all_run.py`; `erase_masks` + `island_geo_data` now ship in make_tarball — earlier cloud bakes silently lacked them); shelf-edge depth taper (5 islands' 50-80-block boundary walls → 0.00 vs the Y-17 target); footprint enclosed-hole fill (Ouvea's lagoon renders from DEM; rule mirrored in `render_drive._content_tiles` / `render_islands._footprint_tiles_from_height` / `world_map_baked` — the last also fixed a cross-vs-square dilation drift); ownership manifest `islands/region_ownership_s101.json` + mainland skip-list `cloud_bake/mainland_skip_regions_s101.txt` (206 regions, verified 0 mainland land). (3) **Splice facts (verified in code + decoded chunks):** the mainland's rendered deep floor is **Y-17** via `ocean_decorator` `coastline_shape.max_depth_for_reshape=80`, NOT the spline — the old -14..-30 deep-spline plan is DROPPED (user approved -17 as-is); `islands/install_sparse.py` is the ONLY mainland-safe merger (`install_islands.crop_merge_install` overwrites destination regions — never use on a combined world); every pre-S101 `islands/out/` render is at STALE offsets/masks (old Madre overlaps 13 mainland LAND regions) — swept at V9. (4) **validate_3x3 RETIRED (user decision S101)** — iterate via single-tile `validate_test_tile.py` (carve_rivers 4-tuple unpack FIXED S101; still derives its own legacy river_water_y — align someday) + `render_single_tile.sh` + topdown MCA renders; ignore 3×3/baseline sections below, they are historical. **NEXT: V9 island render (`islands/_cloud_render_v9.sh`) → user walk; island clearings design (islands have NO windthrow/floodplain masks — gaps 2/4 never fire there; user wants DEM-derived synth, needs sign-off); mainland render prep (skip-list wiring into render_50k_final.sh, bed-cache FORCE-REGEN — stale vs S100 override, snapshot mask refresh or per-box scp, mainland-first + install_sparse combined-world assembler).** — S94 state below is historical.

**S94 prior state:** — RIVER RENDERING OVERHAUL built + mostly user-walked. [memory/S94_handoff.md](memory/S94_handoff.md). Six new gated passes replace the old per-cell water cap (which caused a lengthwise "split down the middle" on wide channels): (1) **flood-settle** (`river_carve.flood_settle`, `core/river_flood_settle.py`) — MC glass-platform sim: flat terraced water, spill-contained (0 levees), lake-safe (`land=` MUST exclude lakes — drained (62,61) to −15 if not), lumpy bed kept; (2) **bank-taper** (`river_carve.bank_taper`, `core/bank_taper.py`) — trough walls → gentle gaussian valley, flush-W shore + W+1 ring, scale-with-height; (3) **step-limit (A)** — ≤1-block bank steps; (4) **despike** (`bank_taper.despike_rock`) — thin "stonehenge" rock columns flattened, broad outcrops kept; (5) **rock-poke paint** (`core/surface_decorator.py:paint_river_rocks`) — emergent bed → litho dark band; (6) **B dirt risers** (`core/chunk_writer.py`, `_BANK_SOIL_RADIUS=12`) — deep soil on bank cells so step faces show dirt not stone. USER WALKED + APPROVED water + banks; **despike pending final walk**. (84,60) installed to Vandir50k_verify. **PENDING (original S94 goals, NOT done): biome-roster walk + validate_3x3 baseline re-sync; before 50k do a real river-crossing seam-pair walk + perf profile.** Prior S93 (`1baa47b`): headwater taper OFF, [memory/S93_handoff.md](memory/S93_handoff.md). All S93 ships remain LIVE: monotone water quantization, lake-bowl warp, canyon guard (`bed_max_extra_depth`), bed-cache v17→v19 race-safe migration (NEVER delete v17; box log must show MIGRATED/HIT), cross-tile ecotone (GC+species), razor-seam halo'd soften, windowed 8k sampling, S93c/d user-confirmed. Taper machinery parked behind `river_carve.headwater_taper.enabled` — re-attempt ONLY inside the dedicated carver cross-section session (incision on slopes = carve shape problem, not water level; 2-strike history in handoff). **STALE-TILE NOTE: verify-world tiles (79,71),(27,33),(27,34),(62,61),(19,76) still carry tapered water until re-rendered (flag-off renders sweep them clean).** Box snapshot `vandir-baked-s93` (396927540) = render-ready (S89 rock masks + v19 bed; the S86 snapshot lacks them → rock silently OFF).
