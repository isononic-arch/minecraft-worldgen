...
**Current state:** Session 90 (2026-06-10) — **READ [memory/S90_handoff.md](memory/S90_handoff.md) FIRST.** Branch `s85-cherry-picks` tip `52d5a77`. DONE+pushed+walked: 73|74 height seam fixed (drift_fill), organic lake shoreline (signed-distance smooth + 2-octave domain warp → de-staircased Balmorhea→Nasworthy), lake depth from GLOBAL EDT (gradual deep bowl ~35 med/~50 huge, no bottom-out), coastal land lift off Y63 (`terrain_spline.mc_y_out[4]` 64→67), ocean beaches verified preserved. **OPEN REGRESSIONS — fix before 50k:** (1) underwater tile-boundary seams at underwater coastlines (ocean-depth `distance_transform_edt` in `core/column_generator.py`:~1032 is per-tile → halo it); (2) world-wide SURFACE-GENERATOR seams = trees/schematics/vegetation coverage at tile borders, NOT terrain (per-tile RNG/field → world-coord splitmix64/halo; ASSESS first via SURF_DUMP sblk diff); (3) beaches propagate too far (tighten `eco_gradients` beach `_dither_width`); (4) beaches should apply at Y64 OR Y65; (5) lake "retaining wall" + trench at lakeshores (LAKES only, not beaches → S90 lake carve lacks rim-smoothing; proud basin rim + dry low ring). Boxes ALL DEAD (0 billing). Old lake `_bowl` branches dormant (delete after in-world confirm). Full S90 detail + TPs + infra notes in the handoff. — Prior S89 state below.
