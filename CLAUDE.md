...
**Current state:** Session 57 (2026-04-16) — **Phase 3a: Meadow clearings + boreal moss + S57 Phase 2.75e.** Phase 2.75e: eco overlay loop disabled (eliminates banding for all biomes), GrassTerrace archived, zone 40 MC biome inheritance fixed (override_tile detection), grass cutoff at elevation (Y 325→350 biome-agnostic fade), packed_ice purged from noise_layers_biome subsurface. Phase 3a: `meadow_clearing_field` wired into pipeline — shared low-freq noise field (wavelength 256 blocks) drives both surface_decorator and schematic_placement. Inside forest clearings (6 temperate biomes + gap==4 floodplain): surface forest-floor blocks (podzol/dirt/moss/rooted_dirt/coarse_dirt) → grass_block; ground cover → short_grass dominant + trace flowers; trees fully suppressed. At clearing seam: probabilistic surface shift, tall_grass band, tree density × 0.4. Boreal moss carpet scales with concavity_norm (denser in basins); moss excluded from SNOWY_BOREAL_TAIGA/ARCTIC_TUNDRA/FROZEN_FLATS (blocks snow_layer accumulation). **Carry-forward:** Phase 3b cross-tile ecotone (seam awareness), rock lithology palette softening (final cleanup pass), SEMI_ARID_SHRUBLAND sand patches inland, desert_pavement ground cover, Phase B height.tif regen, aspect convention drift, world_studio.py duplicate BIOME_TO_MC, snow tuning.
