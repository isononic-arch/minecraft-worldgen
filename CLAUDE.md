...
**Current state:** Session 93 CLOSED (2026-06-12) — **`1baa47b`: headwater taper OFF in config (user verdict: keep the original WIDE rivers; the thin-stream look reads as trenches on slopes). Flag-off (30,12) re-rendered BYTE-IDENTICAL to pre-taper water + installed. READ [memory/S93_handoff.md](memory/S93_handoff.md) FIRST.** All other S93 ships remain LIVE: monotone water quantization, lake-bowl warp, canyon guard (`bed_max_extra_depth`), bed-cache v17→v19 race-safe migration (NEVER delete v17; box log must show MIGRATED/HIT), cross-tile ecotone (GC+species), razor-seam halo'd soften, windowed 8k sampling, S93c/d user-confirmed. Taper machinery parked behind `river_carve.headwater_taper.enabled` — re-attempt ONLY inside the dedicated carver cross-section session (incision on slopes = carve shape problem, not water level; 2-strike history in handoff). **STALE-TILE NOTE: verify-world tiles (79,71),(27,33),(27,34),(62,61),(19,76) still carry tapered water until re-rendered (flag-off renders sweep them clean).** Box snapshot `vandir-baked-s93` (396927540) = render-ready (S89 rock masks + v19 bed; the S86 snapshot lacks them → rock silently OFF).
