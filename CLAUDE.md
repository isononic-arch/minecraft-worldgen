...
**Current state:** Session 47 (2026-04-12) — **Phase 1.75 COMPLETE + surface decorator gating.** Geology flag ON (`lithology.feature_flag_enabled: true`). All 4 sublayers working in-game: deepslate bedrock band, lithology-palette basement banding (variable 4-10 block thickness + XZ waviness + per-column ±3Y noise), flow-driven sediment, slope-driven soil horizon. Surface decorator gated: when geology ON, rock painting (gap==5) uses naturalistic soil palette (grass/coarse_dirt/gravel/packed_mud by slope+concavity) instead of legacy terracotta/basalt/stratification mesh. Lithology read coordinate bug fixed (was reading at full-res offsets from 1:8 mask). 46/46 unit tests, 3×3 baselines green on 48_48 (flag-OFF regression) and 24_80/59_53 (flag-ON visual). In-game verified on tiles 24_80 (sedimentary+deepslate_metamorphic, 124-block cliff faces) and 59_53 (temperate rainforest). **NEXT (S48):** Extend surface decorator gating to ALL biomes (currently only gap==5 rock). Sublayers 5–6 (river_bed, lake_bed) deferred until callers thread `river_meta`/`hydro_lake`. Palette tuning pass on all 6 lithology groups. **Carry-forward:** aspect convention drift (eco_gradients vs wind_model sign), 51_53 flag-ON shadow hookup.
