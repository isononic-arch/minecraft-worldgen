...
PYEOF
  ...
  cmd+="python3 tools/build_terrain_derived.py --only rock_layers,cap --scale 8 >> /root/render_build.log 2>&1; "
  cmd+="python3 tools/build_terrain_derived.py --only talus --scale 2 >> /root/render_build.log 2>&1; "
