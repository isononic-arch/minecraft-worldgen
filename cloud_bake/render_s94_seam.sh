#!/usr/bin/env bash
# render_s94_seam.sh — S94 SEAM-focused validation render, FULL box lifecycle.
#
# Identical lifecycle to render_s94_validation.sh (auto-killer -> dispatch ->
# on-box health -> collect+md5 -> local health -> delete -> verify 0 -> install
# on all-gates-pass) EXCEPT each box renders its assigned tiles CONCURRENTLY via
# run_pipeline --tile-list + --threads (the proven render_50k.sh per-box
# parallelism), instead of one-tile-per-call sequentially. With parallel
# dispatch a box finishes in ~max(single-tile time), so NBOX is chosen for
# MEMORY safety (fewer concurrent tiles/box), not speed. Doubles as the per-tile
# perf+memory profile of the new river passes before the 8-box 50k.
#
# BED-CACHE LAW: never touches masks/_bed_cache_v17.pkl. Health asserts the log
# shows bed cache MIGRATED/HIT and never a rebuild.
#
# Usage:  NBOX=4 bash cloud_bake/render_s94_seam.sh
# Env: NBOX(4) TILES(memory/verify_s94_seam_tiles.txt) OUT_DIR(/d/s94_seam_out)
#   DEST(verify world region/) THREADS(40) OMP(2) TTL_MIN(150) LOC(fsn1)
set -u
TOKEN_FILE="/c/Users/nicho/.hetzner_token"
IDS_FILE="/c/Users/nicho/.hetzner_ids"
SNAPSHOT_ID="${SNAPSHOT_ID:-396927540}"   # vandir-baked-s93 (render-ready)
NBOX="${NBOX:-4}"
TILES="${TILES:-memory/verify_s94_seam_tiles.txt}"
OUT_DIR="${OUT_DIR:-/d/s94_seam_out}"
DEST="${DEST:-/d/modrinth_vandir/saves/Vandir50k_verify/region}"
BRANCH="s85-cherry-picks"
THREADS="${THREADS:-40}"; OMP="${OMP:-2}"
TTL_MIN="${TTL_MIN:-150}"
LOC="${LOC:-fsn1}"
...
log "S94 SEAM validation: $NT tiles on $NBOX boxes (parallel) from snapshot $SNAPSHOT_ID"
...
for i in $(seq 1 "$NBOX"); do
  resp=$(hz -X POST "$API/servers" -d "{\"name\":\"vandir-s94seam-$i\",\"server_type\":\"ccx63\",\"image\":$SNAPSHOT_ID,\"location\":\"$LOC\",\"ssh_keys\":[$KEY_IDS],\"start_after_create\":true}")
  ...
  IDS+=("$id"); IPS+=("$ip")
  log "created vandir-s94seam-$i id=$id ip=$ip"
done
printf "%s\n" "${IDS[@]}" > "$IDS_FILE"
...
# ---- 4. dispatch (PARALLEL per box via --tile-list) ----------------------
...
dispatch(){
  ...
  local lf="$OUT_DIR/render_s94seam_box${b}.log"; > "$lf"
  ...
  # box's own tile list (for on-box health check) + the --tile-list argument
  ...
  for t in "${TS[@]}"; do
    [ -z "${t// }" ] && continue
    local tx=${t%% *} tz=${t##* }
    tlist="${tlist:+$tlist;}${tx},${tz}"
  done
  # NOTE: no cache clear (bed-cache law), no mask upload (snapshot current),
  # no derived-mask build (snapshot ships S89 masks = render-ready), flags
  # committed ON. One run_pipeline call -> ProcessPoolExecutor renders all the
  # box's tiles concurrently (THREADS workers, OMP per-proc numpy threads).
  ...
  cmd+="python run_pipeline.py --config config/thresholds.json --masks masks/ --schem-index schematic_index.json --output output/ --tile-list \"$tlist\" --threads $THREADS >> /root/rv_box.log 2>&1; "
  ...
  log "box$b ($ip) dispatched tile-list: $tlist"
}
...
for b in $(seq 0 $((NBOX-1))); do
  ...
  if grep -q "RESULT: PASS" "$OUT_DIR/health_box${b}.txt"; then
    ...
    log "box$b health: FAIL — see $OUT_DIR/health_box${b}.txt"; HEALTH_FAIL=1
  fi
  if ssh root@"$ip" "grep -lE '_BedCacheRefusal|falling back to rebuild' /root/rv_*.log" 2>/dev/null | grep -q .; then
    log "box$b BED CACHE VIOLATION — rivers untrustworthy"; HEALTH_FAIL=1
  fi
done
...
LOCAL_RC=$?
tail -6 "$OUT_DIR/health_local.txt"
...
if [ "$LOCAL_RC" = "0" ] && [ "$MD5_FAIL" = "0" ] && [ "$HEALTH_FAIL" = "0" ]; then
  ...
  log "S94 SEAM RENDER: ALL GATES PASS"
else
  log "S94 SEAM RENDER: GATES FAILED (local_rc=$LOCAL_RC md5_fail=$MD5_FAIL box_health_fail=$HEALTH_FAIL) — NOT installed. Outputs kept in $OUT_DIR"
fi
