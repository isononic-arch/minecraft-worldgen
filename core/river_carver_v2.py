...
def carve_rivers(
    ...
    # S79: connectivity layer ARCHIVED (not deleted) per user request.
    # The lake outflow/inflow drawing pass is gated off; rivers and lakes
    # render naturally without artificial connectors.  Will be rebuilt
    # ground-up in a future session.  To re-enable, flip the flag below.
    _CONNECTIVITY_ENABLED = False
    if _CONNECTIVITY_ENABLED and lake_water.any() and centerline.any():
