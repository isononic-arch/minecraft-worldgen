...
- **S89 (2026-05-31/06-03)**: **Full physical rock + snow + vegetation surface overhaul; FLAGS FLIPPED ON, merged to master.** Branch `s85-cherry-picks`. Rock: rock_layers slope tiers (40/45/50°, 6 per-group palettes), foliation ribs (real flow-line cross-cutting, contrast-swap), flow-proportional washes, talus/cliff_cap/varnish, base + subtle snow-zone relief. **Snow rebuilt from a flat altitude line → realistic snowline**: per-biome `snow_lines` (climate-ordered: snowy ~400 / BA 540 / BT 600 / temperate 625-650 / dry 655-760) + aspect (south higher, north lower) + convexity (ridges higher, gullies lower) + **patchy micro-terrain transition band** (survival = altitude + concavity + shade + `snow_potential` drifts/couloirs = Winstral Sx + curvature). Gaea base + depth-snow gully fingers + carpet + `frozen_peaks` runtime (was stony_peaks → melted snow). Vegetation: tree SINK (no floaters/flagpoles), real-height-gated krummholz tiny pines (size codes mislabeled), ecotone conifer-only filter (no birch in snow), per-biome canopy-radius density, killed `scotsp_b_sm` stripped-oak tree, grass terraces on rock benches. ~13 cloud renders, 6 litho/alpine tiles validated. **NEXT: full-biome + mountain + beach validation sweep, then 50k regen.** See [memory/S89_handoff.md](memory/S89_handoff.md).
