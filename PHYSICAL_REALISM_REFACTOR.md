...
### Phase 2.75 — Cross-biome + environment layers [Updated S51]

**Scope:** Revised S50/S51 — dropped `temperate_riparian_fringe` and `lake_edge` (existing signals sufficient). Beach and snow_cap_north converted from surface pipeline layers to precompute masks (S51).
...
- `beach` — **Converted S51:** precompute mask `beach.tif` via `rebuild_beach.py`. EDT from ocean at 1:8, tight elevation + slope gate. Y=63 constraint enforced in eco_gradients (gap==9). Sand-only surface, no gravel/podzol dither.
...
**Exit criteria:** snow_cap_north visible on high-elevation north faces (via precompute mask). beach places sand at Y=63 coastlines. desert_pavement breaks up arid monotone on (24,80). diag_layer_ownership.py produces real output.
