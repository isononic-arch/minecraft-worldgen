...
5. **Lake "retaining wall" + trench at lakeshores (LAKES ONLY — ocean beaches clean, so NOT the spline lift; it's the S90 lake carve).** A raised lip/berm rings the lake, higher than both water and the land behind it, with an unnatural trench/swale between berm and natural slope — doesn't trace terrain, looks engineered. Cause: `core/river_carver_v2.py:_carve_lakes_v2` has **NO rim-smoothing pass** (designed, left disabled in v1). The warped/terrain-followed water mask sits inside the natural basin; the only-lower carve leaves the natural basin **rim proud (wall)** and the un-flooded low basin-floor between the shrunk waterline and the rim reads as a **trench** (old terrain-intersection carve flooded the whole below-spill basin → no dry ring → v2 regression). Suspects: water mask (`sdf>shore_thresh`) shrinking inward vs natural basin; `terrain_follow` over-subtracting near rim; only-lower never lowering the proud rim; **missing rim-smoothing**. FIX: add halo-aware rim-smoothing at the lake edge (gaussian on surface_y over a dilated shore ring, seam-safe — padded-halo pattern) to dissolve the wall + grade the trench, and/or extend the water mask to the natural basin extent. DIAGNOSE: cross-section surface_y perpendicular to a lake shore (33,33) — expect `water | dry-low-ring (trench) | proud rim (wall) | land`; toggle `LAKE_V2_OFF=1` to confirm it's v2.
