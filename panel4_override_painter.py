...
class Panel4Window(QMainWindow):
    ...
    def _build_ui(self):
        ...
        self.btn_river = QPushButton("🌊  Paint Rivers (val 80)")
        self.btn_river.setStyleSheet("""
            QPushButton { background: #1a3f6f; color: white; font-size: 12px;
                          font-weight: bold; padding: 8px; border-radius: 4px; }
            QPushButton:hover { background: #2a5f9f; }
            QPushButton:disabled { background: #333; color: #666; }
        """)
        self.btn_river.setEnabled(False)
        self.btn_river.setToolTip(
            "Auto-paint val 80 Riparian Woodland on river channels.\n"
            "Uses current River threshold and Land threshold slider values.\n"
            "Enable flow overlay first to preview what will be painted."
        )
        ctrl_layout.addWidget(self.btn_river)
    ...
    def _connect_signals(self):
        ...
        self.btn_river.clicked.connect(self._paint_rivers)
    ...
    def _load_data(self, ov_path: str, ht_path: str | None, co_path: str | None, fl_path: str | None = None):
        ...
        self.btn_river.setEnabled(self.flow_data is not None)
    ...
    def _paint_rivers(self):
        """
        Auto-paint val 80 (Riparian Woodland) on river channel pixels.
        Uses identical land mask logic to the flow overlay display.
        Variable width by flow intensity + elevation. Noise filtered.
        """
        from scipy import ndimage

        MIN_LEN = 10  # minimum connected component length in display pixels

        if self.override_data is None or self.flow_data is None or self.height_data is None:
            QMessageBox.warning(self, "River Pass",
                "Flow data and height data must both be loaded.")
            return

        reply = QMessageBox.question(
            self, "Paint Rivers",
            f"Auto-paint val 80 (Riparian Woodland) on river channels.\n\n"
            f"Settings:\n"
            f"  River threshold: {self._flow_threshold}%\n"
            f"  Land threshold:  {self._land_threshold}%\n"
            f"  Min river length: {MIN_LEN}px\n\n"
            f"Tip: enable the flow overlay to preview before running.\n\n"
            f"Continue?",
        ...
        self._push_undo()
        self.status.showMessage("Painting rivers...")
        QApplication.processEvents()

        flow_thresh = self._flow_threshold / 100.0
        land_thresh = self._land_threshold / 100.0

        # Land mask — identical to flow overlay and brush constraint
        land = self.height_data > land_thresh

        # River candidates: flow above threshold, on land only
        river_candidates = (self.flow_data > flow_thresh) & land

        # Label connected components and filter by minimum length
        labeled, num_features = ndimage.label(river_candidates)
        if num_features == 0:
            QMessageBox.information(self, "River Pass",
                "No river pixels found at current threshold.\n"
                "Try lowering the River threshold slider.")
            return

        component_sizes = ndimage.sum(
            river_candidates, labeled, range(1, num_features + 1)
        )
        valid_labels = [i + 1 for i, s in enumerate(component_sizes) if s >= MIN_LEN]

        if not valid_labels:
            QMessageBox.information(self, "River Pass",
                f"No river components passed the length filter.\n"
                f"All components were shorter than {MIN_LEN}px.\n"
                "Try lowering the River threshold slider.")
            return

        # Filtered river skeleton — all valid channel pixels
        river_skeleton = np.isin(labeled, valid_labels)

        # Variable width: build a continuous dilated river layer.
        # Strategy: dilate the FULL skeleton by a base radius, then
        # additionally dilate high-flow lowland pixels by extra radius.
        # This preserves channel continuity vs per-pixel dilation.
        struct = ndimage.generate_binary_structure(2, 1)

        # Base dilation — 1px on all valid river pixels (minimum width)
        base_dilated = ndimage.binary_dilation(river_skeleton, structure=struct, iterations=1)

        # Mid-flow dilation — extra 1px where flow is moderate+
        mid_flow = river_skeleton & (self.flow_data > flow_thresh * 1.5)
        mid_dilated = ndimage.binary_dilation(mid_flow, structure=struct, iterations=1)

        # High-flow lowland dilation — extra 2px where flow is high AND elevation is low
        # height_data > land_thresh means high value = low elevation land
        low_elev = self.height_data > (land_thresh + 0.3)
        high_flow = river_skeleton & (self.flow_data > flow_thresh * 2.5) & low_elev
        high_dilated = ndimage.binary_dilation(high_flow, structure=struct, iterations=2)

        # Combine all dilation layers, constrain to land
        to_paint = (base_dilated | mid_dilated | high_dilated) & land

        painted = self.override_data.copy()
        painted[to_paint] = 80

        new_pixels = int(np.sum(painted == 80)) - int(np.sum(self.override_data == 80))
        self.override_data = painted
        self._refresh_display()
        self.status.showMessage(
            f"River pass complete — {new_pixels:,} new val 80 pixels painted. "
            f"Ctrl+Z to undo if needed."
        )
