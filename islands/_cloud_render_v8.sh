#!/usr/bin/env bash
# S98 V8 FULL render — all 15 islands, 8 boxes (max), Grenada-treatment pre-rotation
...
TGZ="/tmp/rv_code.tgz"
COL="$ROOT/islands/_collect_v8"; mkdir -p "$COL"
...
NAME=(v8-b1 v8-b2 v8-b3 v8-b4 v8-b5 v8-b6 v8-b7 v8-b8)
...
# ---- 5. safety sweep ----
rem=$(api "$API/servers" | "$PY" -c "import sys,json;[print(s['id']) for s in json.load(sys.stdin).get('servers',[]) if s['name'].startswith('v8-b')]" 2>/dev/null)
