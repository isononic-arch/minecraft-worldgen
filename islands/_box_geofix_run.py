"""_box_geofix_run.py — on-box: RE-BAKE the 3 islands with the geology fixes
(linear band-pass flow, no wash hack, wide smoothstep shelf), then render land +
shelf-buffer tiles WITH TREES (real pass). Verifies washes-thin / tiers-vary /
no-seams / shelf / trees. Writes /root/geofix_done. Run from /root/minecraft-worldgen
in tmux.
"""
...
def main():
    t0 = time.time()
    print("=== RE-BAKE 3 islands (linear flow + smoothstep shelf) ===", flush=True)
    ...
    Path("/root/geofix_done").write_text(f"done {(time.time()-t0)/60:.1f}m, {len(jobs)} tiles\n")
    print(f"DONE_GEOFIX ({(time.time()-t0)/60:.1f}m)", flush=True)
